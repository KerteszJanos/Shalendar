...
<div class="lists-container">
    ...
    <div class="lists-content" v-if="calendarLists.length > 0">
        <div v-for="list in calendarLists" :key="list.id" class="list-item" :style="{ backgroundColor: list.color || '#CCCCCC' }">
            ...
            <!-- Ticketek megjelenítése draggable komponenssel -->
            <div class="ticket-list" v-if="list.tickets && list.tickets.length > 0">
                <draggable v-model="list.tickets" @end="onTicketDragEnd(list)" :group="{ name: 'tickets', pull: false, put: false }" itemKey="id">
    ...
    setup() {
        ...
        const addCalendarList = async () => {
            try {
                ...
                // Az új lista legyen tickets tömbbel (üres)
                const newListData = {
                    ...response.data,
                    tickets: []
                };
        ...
        const fetchCalendarLists = async () => {
            try {
                ...
                calendarLists.value = response.data.map(list => ({
                    ...list,
                    tickets: (list.tickets || []).sort((a, b) => Number(a.currentPosition) - Number(b.currentPosition))
                }));
            } catch (error) {
        ...
        const updateTicketOrder = async (list) => {
            // Újraszámoljuk a ticketek pozícióját (1-től)
            ...
            try {
                ...
                // Explicit újrarendezés: hozzárendeljük a rendezett tömböt a Vue reaktivitás miatt
                list.tickets = [...list.tickets].sort((a, b) => Number(a.currentPosition) - Number(b.currentPosition));
            } catch (error) {
        ...
        // Drag and drop esemény: az adott lista ticketjeinek újrarendezése
